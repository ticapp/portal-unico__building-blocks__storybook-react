...
export const Checkbox = ({ className, disabled = false, label, labelPosition = 'after', size = 'md', ...props }: CheckboxProps) => {
  ...
  return <div className={cssCheckbox}>
    <Stack direction='horizontal'>
      <input type="checkbox" checked={checked} ref={focusRef} onFocus={() => onFocusHandler(true)} onBlur={() => onFocusHandler(false)} disabled={disabled} aria-label={'checkbox'}/>
      {
        labelPosition === 'before' &&  <label id={labelId} htmlFor={checkboxId}>{label}</label>
      }
      <div className='icons' id={checkboxId} onMouseDown={focus} >
        {iconBuilder()}
      </div>
      {
        labelPosition === 'after' &&  <label id={labelId} htmlFor={checkboxId}>{label}</label>
      }
    </Stack>
  </div>
}
